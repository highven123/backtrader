...
from data_handler import get_data, get_data_with_cache, fetch_crypto_data
...
# 兼容旧接口，统一走新版 data_handler

def get_stock_data(symbol, start_date, end_date, data_source='akshare'):
    """兼容旧接口，实际调用新版 data_handler 的 fetch_stock_data 或 get_data"""
    return get_data('沪深A股', symbol, start_date, end_date)

def get_stock_data_yfinance(symbol, start_date, end_date):
    """兼容旧接口，实际调用新版 data_handler 的 get_data，指定 yfinance"""
    return get_data('沪深A股', symbol, start_date, end_date, fx_data_source='yfinance')
...
def main():
    ...
    with tab1:
        ...
        if market == "美股":
            ...
            symbol_hint = "如：EURUSD、USDJPY、GBPUSD"
            # 增加常用外汇对下拉选择
            fx_pairs = [
        ...
        # 外汇市场 symbol 用下拉选择，其他市场用文本输入
        if market == "外汇":
            symbol = st.sidebar.selectbox("标的代码", fx_pairs, index=fx_pairs.index(symbol_default), help=symbol_hint)
        else:
        ...
        # 数据源选择（外汇和加密货币市场支持多数据源）
        fx_data_sources = ["yfinance", "akshare", "alphavantage"]
        crypto_data_sources = ["ccxt", "akshare", "okx"]
        if market == "外汇":
            data_source = st.sidebar.selectbox("外汇数据源", fx_data_sources, index=fx_data_sources.index("yfinance"))
        elif market == "加密货币":
            data_source = st.sidebar.selectbox("加密货币数据源", crypto_data_sources, index=crypto_data_sources.index("ccxt"))
        
        # 杠杆和多空方向参数（仅外汇和加密货币市场显示）
        leverage = 1
        trade_side = '多头'
        trade_mode = '单方向'
        if market in ["加密货币", "外汇"]:
            trade_side = st.sidebar.selectbox("交易方向", ["多头", "空头", "多空混合"], help="选择开多、开空或多空混合")
            if trade_side == "多空混合":
                trade_mode = st.sidebar.selectbox("混合模式", ["反向开仓", "仅平仓"], help="遇到反向信号时的处理方式")
        
        # 策略参数补充数据源、杠杆和方向
        strategy_params['data_source'] = data_source
        strategy_params['leverage'] = leverage
        strategy_params['trade_side'] = trade_side
        strategy_params['trade_mode'] = trade_mode
